...
- EXISTS 방식: 조건을 만족하는 행을 찾으면 탐색을 중단할 수 있음

실제 이점은 데이터 분포와 인덱스, 옵티마이저의 실행 계획에 따라 달라집니다. 따라서 `EXISTS`가 항상 `JOIN`보다 빠르다고 일반화하지 않고, 이 프로젝트의 쿼리를 동일한 조건에서 측정한 결과로 판단했습니다.
