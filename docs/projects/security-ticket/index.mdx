...
- 기본 Form Login은 폼 파라미터 기반이어서 프로젝트의 JSON 요청 형식과 맞지 않음
- 계정 잠금, 실패 횟수 증가, 첫 로그인 체크 등 비즈니스 로직 통합
- Admin/Customer 타입별 다른 응답 포맷

- 프론트엔드와 JSON API 계약을 사용하고 있었기 때문에 로그인만 폼 전송 방식으로 바꾸면 요청 형식과 오류 응답 규칙이 달라집니다.
- JSON을 읽는 커스텀 인증 필터도 대안이지만, 이 프로젝트에서는 계정 상태 변경과 최초 로그인 응답을 서비스 계층에서 명시적으로 다루기 위해 Controller 진입 방식을 선택했습니다.
...
- 로그인 진입점을 Controller로 구성하면서 Spring Security 표준 컴포넌트를 직접 연결한 경험
