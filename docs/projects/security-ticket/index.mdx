...
- **핵심 설계 원칙**
  ...
  - **인증/인가**: Spring Security 기반 역할별 접근 제어
...
### 1. JSON 로그인 및 계정 정책 처리

- 프로젝트의 JSON API 형식에 맞춰 로그인 엔드포인트 구성
- 계정 잠금과 최초 로그인 여부 등 계정 정책을 서비스 계층에서 처리
- Admin/Customer 역할에 따른 접근 권한과 응답 구분
...
- ✅ JSON 로그인 엔드포인트와 계정 잠금·최초 로그인 정책 구현
