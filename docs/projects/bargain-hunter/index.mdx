...
| **팀 구성** | 5인 (BE 4인 / FE 1인)              |
| **역할**    | 인증·인가 아키텍처 및 사용자 도메인 담당 |

### 주요 기능

- 🗺️ **지도 탐색**: TourAPI 연동 관광지 정보 제공
- 📝 **리뷰 시스템**: 관광지 리뷰 등록 및 조회
- 💬 **가격 질의**: Gemini API 챗봇
...
- ✅ PKCE 적용으로 **Authorization Code 탈취 위험 완화**
...
- ✅ 인증 검증 지점을 Gateway로 일원화 → 각 서비스의 중복 JWT 파싱·검증 부담 축소
...
### 1️⃣ 외부 I/O 분리를 통한 이메일 인증 요청 경로 개선 [(관련 PR 보기)](https://github.com/JocketDan/jocketdanBackend/pull/34)

**문제**: SMTP 통신이 사용자 요청 처리 경로를 점유해, 인증 요청 API가 메일 발송 완료까지 대기

**개선**: Spring Event와 @Async를 활용해 메일 발송을 비동기 후처리로 분리하고, 인증 요청 API가 SMTP 처리 완료를 기다리지 않고 응답하도록 구성
...
- ✅ SMTP 통신을 요청 경로에서 분리해 인증 API가 메일 발송 완료를 기다리지 않도록 개선 (평균 2.5s → 0.2s)
- ✅ Redis HINCRBY 원자적 연산으로 동시성 제어
- ✅ Gateway 중앙 인증으로 각 서비스의 JWT 파싱 오버헤드 제거

### 보안 강화

- ✅ PKCE 적용으로 OAuth2 Authorization Code 탈취 위험 완화
